...
<div class="tailwind-scope mt-4"><h2 class="text-2xl font-bold text-gray-100 mb-6">Withdrawal History</h2>
    <div class="bg-[#1a222d] border border-[#334155] rounded-lg overflow-hidden">
        <table class="w-full table-custom"><thead><tr><th>Date</th><th>Requested Amount</th><th>Charges</th><th>Net Payable</th><th>Status</th></tr></thead>
        <tbody>@forelse($withdrawals as $w) 
        @php 
        ...
        <tr>
            ...
            <td class="font-bold text-gray-400">${{ number_format($w->amount, 2) }}</td>
            ...
            <td><span class="text-xs px-2 py-1 rounded bg-{{ $w->status=='approved'?'green':($w->status=='rejected'?'red':'yellow') }}-900 text-{{ $w->status=='approved'?'green':($w->status=='rejected'?'red':'yellow') }}-300">{{ ucfirst($w->status) }}</span></td>
        </tr> 
        @empty 
        <tr><td colspan="5" class="text-center p-8 text-gray-500">No withdrawals yet</td></tr> 
