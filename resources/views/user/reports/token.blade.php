...
<div class="tailwind-scope mt-4 max-w-[1400px] mx-auto">
    <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
            <h2 class="text-2xl font-bold text-gray-100">Token Distribution Report</h2>
            <p class="text-gray-400 text-sm">Detailed ledger of all received tokens.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div class="bg-[#1a222d] border border-[#334155] p-5 rounded-xl shadow-lg">
            <h3 class="text-gray-400 font-medium mb-1 text-xs uppercase">Lifetime NEXA 1.0 ({{ strtoupper($tokenName) }})</h3>
            <div class="text-2xl font-bold text-blue-400">{{ number_format($totalUtility, 2) }}</div>
        </div>
        <div class="bg-[#1a222d] border border-[#334155] p-5 rounded-xl shadow-lg">
            <h3 class="text-gray-400 font-medium mb-1 text-xs uppercase">Lifetime NEXA 2.0 (RT)</h3>
            ...
            <table class="w-full table-custom">
                <thead>
                    <tr>
                        ...
                        <th class="text-left">Token Type</th>
                        <th class="text-left">Source</th>
                        <th class="text-left">Token Value (USD)</th>
                ...
                <tbody>
                    ...
                    <tr class="transition">
                        ...
                        <td class="text-gray-400">${{ number_format($item->token_value ?? 0, 4) }}</td>
                        <td class="text-right font-bold {{ $item->token_count >= 0 ? 'text-green-400' : 'text-red-400' }}">
                            {{ $item->token_count >= 0 ? '+' : '' }}{{ number_format($item->token_count, 2) }}
                        </td>
                        <td class="text-right">
                            @if($item->status == 'credited')
                                <span class="text-green-400 text-xs"><i class="fa-solid fa-check"></i> Credited</span>
                            @elseif($item->status == 'used')
                                <span class="text-gray-400 text-xs"><i class="fa-solid fa-arrow-right"></i> Used/Converted</span>
                            @else
                                <span class="text-gray-400 text-xs uppercase">{{ $item->status }}</span>
                            @endif
                        </td>
                    </tr>
                    @empty
