...
<style>
  ...
  #tree-viewport{width:100%;overflow:hidden;position:relative;background:#0f172a;border-radius:10px;border:1px solid #334155;height:70vh;min-height:400px;cursor:grab;user-select:none}
  ...
  .node-card{display:flex;flex-direction:column;align-items:center;cursor:pointer;background:#1a222d;border:2px solid #334155;border-radius:10px;padding:10px 14px;min-width:110px;transition:border-color .2s,box-shadow .2s}
  ...
  .node-name{font-size:.75rem;font-weight:600;color:#e2e8f0;margin-top:6px;text-align:center;max-width:100px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
<div class="tailwind-scope mt-4 max-w-[1600px] mx-auto">
  <div class="mb-4 flex flex-wrap items-center gap-3">
    <div>
      <h2 class="text-2xl font-bold text-gray-100">Team Tree</h2>
...
<script>
const ROOT_ID={{ $user->id }},ROOT_NAME=@json($user->name);
...
function setZoom(ns,cx,cy){const r=vp.getBoundingClientRect();cx=cx??r.width/2;cy=cy??r.height/2;const ratio=ns/scale;panX=cx-ratio*(cx-panX);panY=cy-ratio*(cy-panY);scale=Math.min(3,Math.max(0.08,ns));applyT();}
...
function buildNode(node,isRoot){
  ...
  const c=document.createElement('div');c.className='node-card'+(isRoot?' selected':'');
  const bg=node.status==='active'?'#166534':'#7f1d1d';
  c.innerHTML=`<div class="node-avatar" style="background:${bg}">${node.name.charAt(0).toUpperCase()}</div><div class="node-name">${node.name}</div><div class="node-code">${node.referral_code}</div><span class="node-badge ${node.status}">${node.status==='active'?'Active':'Inactive'}</span>${node.children&&node.children.length?`<span class="node-count"><i class="fa-solid fa-users mr-1"></i>${node.children.length}</span>`:''}`;
  if(!isRoot){c.addEventListener('mousedown',e=>e.stopPropagation());c.addEventListener('click',()=>drillDown(node));}
  w.appendChild(c);
...
function loadNode(id){document.getElementById('tree-loader').style.display='block';canvas.innerHTML='';scale=1;panX=0;panY=0;applyT();fetch(`/user/network/tree/node/${id}`).then(r=>r.json()).then(renderTree).catch(()=>{document.getElementById('tree-loader').innerHTML='<span style="color:#f87171">Failed to load.</span>';});}
function renderBC(){const bc=document.getElementById('breadcrumb');bc.innerHTML='';const re=document.createElement('span');re.className='bc-item'+(historyStack.length===0?' current':'');re.textContent=ROOT_NAME+' (You)';if(historyStack.length>0)re.addEventListener('click',()=>{historyStack=[];loadNode(ROOT_ID);document.getElementById('resetTreeBtn').classList.add('hidden');});bc.appendChild(re);historyStack.forEach((item,idx)=>{const s=document.createElement('span');s.className='bc-sep';s.textContent='›';bc.appendChild(s);const el=document.createElement('span');el.className='bc-item'+(idx===historyStack.length-1?' current':'');el.textContent=item.name;if(idx<historyStack.length-1)el.addEventListener('click',()=>{historyStack=historyStack.slice(0,idx+1);loadNode(item.id);});bc.appendChild(el);});}
document.getElementById('resetTreeBtn').addEventListener('click',()=>{historyStack=[];loadNode(ROOT_ID);document.getElementById('resetTreeBtn').classList.add('hidden');});
loadNode(ROOT_ID);
</script>
@endsection
